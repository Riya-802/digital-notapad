...
function DigitalNotepadApp() {
  ...
  function beginStroke(event) {
    // Check if it's a touch event and prevent default scrolling if drawing
    ...
    <div className="app-shell">
      <header className="hero">
        <div className="hero-text">
          ...
          <p className="subtitle">Responsive, native-like mobile layout.</p>
        </div>
      ...
      <main className="dashboard">
        <section className={`panel tool-grid ${!showSettings ? 'mobile-hidden' : ''}`}>
          <div className="tool-section">
            <div className="tool-header">
              <h3>Tools</h3>
              {/* Mobile theme toggle placed inside settings for space */}
              <button className="btn mini-btn mobile-only-inline" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
            ...
            <div className="chip-row">
              {['pen', 'highlighter', 'eraser'].map((item) => (
                ...
                >
                  {item === 'pen' ? '✏️ Pen' : item === 'highlighter' ? '🖍 High.' : '🧽 Eraser'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="form-group">
            <label>Color</label>
            <input type="color" className="color-picker-full" value={penColor} onChange={(e) => setPenColor(e.target.value)} />
          </div>
          <div className="form-group">
        ...
        <section className="panel canvas-section">
          ...
          <div className="editor-grid" style={{ marginBottom: 12 }}>
            ...
            <div className="text-wrap">
              ...
              <div className="chip-row mobile-only-flex" style={{marginTop: '8px'}}>
                 <button className="btn" onClick={startVoiceTyping}>🎙 Voice typing</button>
                 <button className="btn" onClick={() => setText(summarizeText(text))}>✨ Summarize</button>
              </div>
            </div>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search saved notes..." className="search-input" />
      ...
      <div className="mobile-sticky-bar">
        <div className="sticky-scroll-container">
          ...
          <button className={`tool-btn ${tool === 'highlighter' ? 'active' : ''}`} onClick={() => setTool('highlighter')}>🖍</button>
          <button className={`tool-btn ${tool === 'eraser' ? 'active' : ''}`} onClick={() => setTool('eraser')}>🧽</button>
          
          <div className="divider"></div>
          
          <input type="color" className="color-picker-mini" value={penColor} onChange={(e) => setPenColor(e.target.value)} />
          ...
          <button className="btn danger" onClick={clearCanvas}>🧹</button>
...
export default DigitalNotepadApp;
